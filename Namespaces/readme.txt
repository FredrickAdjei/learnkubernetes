...
namespace is mostly reserved for cluster usage, in case that some resources should
be visible and readable publicly throughout the whole cluster. The public aspect of
...
kubectl get namespace - this lists the namespaces

kubectl run nginx --image=nginx --namespace=kube-system - this creates a pod in the
kubesystem namespace

kubectl get pods --namespace=kubesystem - lists pods in the kubesystem namespace

Setting namespace prefrence
kubectl config set-context --current --namespace=<nameofnamespace>
#Validate it
